...
use App\Http\Requests\Panel\Market\BrandRequest;
use App\Http\Services\Image\ImageService;
use App\Http\Services\Panel\Market\BrandService;
...
class BrandController extends Controller
{
    ...
    public function create(CategoryRepo $categoryRepo)
    {
        ...
        return view('panel.market.brand.create', compact(['categories']));
    }
    ...
    public function store(BrandRequest $request, ImageService $imageService): RedirectResponse
    {
        if ($request->hasFile('logo')) {
            $request->image = ShareService::uploadImage($request->file('logo'), 'panel-brand',
                'panel.market.brand.index', $imageService);
        }

        $this->service->store($request);
        return redirect()->route('panel.market.brand.index')->with('swal-success', 'برند با موفقیت اضافه شد.');
    }
    ...
    public function edit(CategoryRepo $categoryRepo, int $id)
    {
        ...
        return view('panel.market.brand.edit', compact(['categories', 'brand']));
    }
    ...
    public function update(BrandRequest $request,ImageService $imageService,int $id): RedirectResponse
    {
        ...
        return redirect()->route('panel.market.brand.index')->with('swal-success', 'برند با موفقیت ویرایش شد.');
    }
    ...
    {
        $this->repo->delete($id);
        return redirect()->route('panel.market.brand.index')->with('swal-success', 'برند با موفقیت حذف شد.');
    }
