...
use App\Http\Repositories\Panel\PostRepo;
use App\Http\Requests\Panel\PostRequest;
use App\Http\Services\Panel\PostService;
...
class PostController extends Controller
{
    ...
    public function store(PostRequest $request, ImageService $imageService): RedirectResponse
    {
        if ($request->hasFile('image')) {
            ...
            if ($result === false) {
                return redirect()->route('panel.post.index')->with('swal-error', 'آپلود تصویر با خطا مواجه شد');
            }
            $request->image = $result;
        }
        $request->published_at = ShareService::dateFix($request->published_at);
        $this->service->store($request);
        return redirect()->route('panel.post.index')->with('swal-success', 'پست با موفقیت ذخیره شد.');
    }
    ...
    public function update(PostRequest $request, ImageService $imageService, int $id): RedirectResponse
    {
        ...
        return redirect()->route('panel.post.index')->with('swal-success', 'پست با موفقیت ویرایش شد.');
    }
    ...
    {
        $this->repo->delete($id);
        return redirect()->route('panel.post.index')->with('swal-success', 'پست با موفقیت حذف شد.');
    }
    ...
    public function changeStatus($id): RedirectResponse
    {
        ...
        return redirect()->route('panel.post.index')->with('swal-success', 'وضعیت پست با موفقیت تغییر کرد.');
    }
}
