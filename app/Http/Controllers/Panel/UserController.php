...
use App\Http\Repositories\Panel\RoleRepo;
use App\Http\Repositories\Panel\UserRepo;
use App\Http\Requests\Panel\UserRequest;
use App\Http\Services\Panel\UserService;
...
class UserController extends Controller
{
    ...
    public function store(UserRequest $request, ImageService $imageService): RedirectResponse
    {
        if ($request->hasFile('profile_photo_path')) {
            $result = ShareService::saveImage($request->file('profile_photo_path'),'panel-user',
                null,$imageService);
            if ($result === false) {
                return redirect()->route('panel.user.index')->with('swal-error', 'آپلود تصویر با خطا مواجه شد');
            }
        ...
        return redirect()->route('panel.user.index')->with('swal-success', 'کاربر با موفقیت ذخیره شد.');
    }
    ...
    public function update(UserRequest $request,ImageService $imageService, int $id): RedirectResponse
    {
        ...
        return redirect()->route('panel.user.index')->with('swal-success', 'کاربر با موفقیت ویرایش شد.');
    }
    ...
    {
        $this->repo->delete($id);
        return redirect()->route('panel.user.index')->with('swal-success', 'کاربر با موفقیت حذف شد.');
    }
    ...
    public function addRoles(int $userId, RoleRepo $roleRepo)
    {
        ...
        return view('panel.user.add-roles', compact(['userId', 'roles']));
    }
...
//        $this->authorize('index', User::class);
        ...
        return redirect()->route('panel.user.index')->with('swal-success', 'نقش با موفقیت به کاربر افزوده شد.');
    }
...
//        $this->authorize('index', User::class);
        ...
        return redirect()->route('panel.user.index')->with('swal-success', 'نقش با موفقیت از کاربر گرفته شد.');
    }
}
