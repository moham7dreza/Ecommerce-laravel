...
use App\Http\Services\Panel\CategoryService;
...
class CategoryController extends Controller
{
    ...
    public function store(CategoryRequest $request, ImageService $imageService): RedirectResponse
    {
        if ($request->hasFile('image')) {
            $request->image = ShareService::uploadImage($request->file('image'), 'panel-post-category',
                'panel.category.index', $imageService);
        }

        $this->service->store($request);
        return redirect()->route('panel.category.index')->with('swal-success', 'دسته بندی با موفقیت اضافه شد.');
    }
    ...
    public function edit(int $id)
    {
        ...
        return view('panel.category.edit', compact(['categories', 'postCategory']));
    }
    ...
    public function update(CategoryRequest $request, ImageService $imageService, int $id): RedirectResponse
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($result === false) {
                return redirect()->route('panel.category.index')->with('swal-error', 'آپلود تصویر با خطا مواجه شد');
            }
        ...
        return redirect()->route('panel.category.index')->with('swal-success', 'دسته بندی با موفقیت ویرایش شد.');
    }
    ...
    {
        $this->repo->delete($id);
        return redirect()->route('panel.category.index')->with('swal-success', 'دسته بندی با موفقیت حذف شد.');
    }
