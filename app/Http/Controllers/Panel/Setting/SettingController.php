...
use App\Http\Repositories\Panel\Setting\SettingRepo;
use App\Http\Requests\Panel\Setting\SettingRequest;
use App\Http\Services\Image\ImageService;
use App\Http\Services\Panel\Setting\SettingService;
...
class SettingController extends Controller
{
    ...
    public function update(SettingRequest $request, ImageService $imageService, int $id): RedirectResponse
    {
        ...
        if ($request->hasFile('logo')) {
            $result = ShareService::saveImageWithName($request->file('logo'), 'panel-setting',
                $setting->title . '-لوگو', $setting->logo, $imageService);
            if ($result === false) {
                return redirect()->route('panel.setting.index')->with('swal-error', 'آپلود تصویر با خطا مواجه شد');
            }
            $request->logo = $result;
        }
        if ($request->hasFile('icon')) {
            $result = ShareService::saveImageWithName($request->file('icon'), 'panel-setting',
        ...
        return redirect()->route('panel.setting.index')->with('swal-success', 'تنظیمات با موفقیت ویرایش شد.');
    }
