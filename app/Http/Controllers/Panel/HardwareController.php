...
use App\Http\Repositories\Panel\HardwareRepo;
use App\Http\Requests\Panel\HardwareRequest;
use App\Http\Services\Image\ImageService;
use App\Http\Services\Panel\HardwareService;
...
class HardwareController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('panel.hardware.index', compact(['hardwares']));
    }
    ...
    public function create(CategoryRepo $categoryRepo, BrandRepo $brandRepo)
    {
        ...
        return view('panel.hardware.create', compact(['categories', 'brands']));
    }
    ...
    public function store(HardwareRequest $request, ImageService $imageService): RedirectResponse
    {
        ...
        return redirect()->route('panel.hardware.index')->with('swal-success', 'سخت افزار با موفقیت ذخیره شد.');
    }
     ...
     * @return Response
     */
    public function edit(CategoryRepo $categoryRepo, BrandRepo $brandRepo, int $id): Response
    {
        $hardware = $this->repo->findById($id);
        $categories = $categoryRepo->getActiveCategories()->get();
        $brands = $brandRepo->index()->get();
        return view('panel.hardware.edit', compact(['hardware', 'categories', 'brands']));
    }
    ...
    public function update(HardwareRequest $request,ImageService $imageService, int $id): RedirectResponse
    {
        ...
        return redirect()->route('panel.hardware.index')->with('swal-success', 'سخت افزار با موفقیت ویرایش شد.');
    }
    ...
    {
        $this->repo->delete($id);
        return redirect()->route('panel.hardware.index')->with('swal-success', 'سخت افزار با موفقیت حذف شد.');
    }
    ...
    public function changeStatus(int $id): RedirectResponse
    {
        ...
        return redirect()->route('panel.hardware.index')->with('swal-success', 'وضعیت سخت افزار با موفقیت تغییر کرد.');
    }
}
