        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.smart-assemble.type.create') }}" class="btn btn-info btn-sm">ایجاد نوع
                        جدید</a>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach ($systemTypes as $systemType)
                            ...
                            <tr>
                                ...
                                <td>
                                    <img
                                        src="{{ asset($systemType->image['indexArray'][$systemType->image['currentImage']] ) }}"
                                        alt="" width="100" height="50">
                                </td>
                                {{--                            <td>{{ $systemType->tags }}</td>--}}
                                <td>
                                    <label>
                                        <input id="{{ $systemType->id }}" onchange="changeStatus({{ $systemType->id }})"
                                               data-url="{{ route('admin.smart-assemble.type.status', $systemType->id) }}"
                                               type="checkbox" @if ($systemType->status === 1)
                                                   checked
                                            @endif>
                                    </label>
                                </td>
                                ...
                                <td class="width-8-rem text-left">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                            <a href="{{ route('admin.smart-assemble.type.gallery.index', [$systemCategory, $systemType]) }}"
                                               class="dropdown-item text-right"><i class="fa fa-images"></i> گالری</a>
                                            <a href="{{ route('admin.smart-assemble.type.edit', $systemType) }}"
                                               class="dropdown-item text-right"><i class="fa fa-edit"></i> ویرایش</a>
                                            <form class="d-inline"
                                                  action="{{ route('admin.smart-assemble.type.destroy', $systemType) }}"
                                                  method="post">
                                                @csrf
                                                @method('Delete')
