        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.smart-assemble.cpu.create') }}" class="btn btn-info btn-sm">ایجاد نسل
                        جدید</a>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach ($systemCpus as $systemCpu)
                            ...
                            <tr>
                                ...
                                <td>
                                    <img
                                        src="{{ asset($systemCpu->image['indexArray'][$systemCpu->image['currentImage']] ) }}"
                                        alt="" width="100" height="50">
                                </td>
                                {{--                            <td>{{ $systemCpu->tags }}</td>--}}
                                <td>
                                    <label>
                                        <input id="{{ $systemCpu->id }}" onchange="changeStatus({{ $systemCpu->id }})"
                                               data-url="{{ route('admin.smart-assemble.type.status', $systemCpu->id) }}"
                                               type="checkbox" @if ($systemCpu->status === 1)
                                                   checked
                                            @endif>
                                    </label>
                                </td>
                                <td class="width-16-rem text-left">
                                    <a href="{{ route('admin.smart-assemble.cpu.edit', $systemCpu->id) }}"
                                       class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                    <form class="d-inline"
                                          action="{{ route('admin.smart-assemble.type.destroy', $systemCpu->id) }}"
                                          method="post">
                                        @csrf
                                        {{ method_field('delete') }}
