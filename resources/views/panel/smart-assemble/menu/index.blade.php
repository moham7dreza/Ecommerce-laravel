        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.smart-assemble.menu.create') }}" class="btn btn-info btn-sm">ایجاد منوی
                        جدید</a>
                                ...
                                <td>
                                    <label>
                                        <input id="{{ $menu->id }}" onchange="changeStatus({{ $menu->id }})"
                                               data-url="{{ route('admin.smart-assemble.menu.status', $menu->id) }}"
                                               type="checkbox" @if ($menu->status === 1)
                                                   checked
                                            @endif>
                                    </label>
                                </td>
                                <td class="width-16-rem text-left">
                                    <a href="{{ route('admin.smart-assemble.menu.edit', $menu->id) }}"
                                       class="btn btn-primary btn-sm"><i class="fa fa-edit"></i>ویرایش</a>
                                    <form class="d-inline"
                                          action="{{ route('admin.smart-assemble.menu.destroy', $menu->id) }}"
                                          method="post">
                                        @csrf
                                        {{ method_field('delete') }}
