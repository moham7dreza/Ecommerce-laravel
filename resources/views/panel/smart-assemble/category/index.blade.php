        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.smart-assemble.category.create') }}" class="btn btn-info btn-sm">ایجاد دسته
                        بندی</a>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach ($systemCategories as $systemCategory)
                            ...
                            <tr>
                                ...
                                <td>
                                    <img
                                        src="{{ asset($systemCategory->image['indexArray'][$systemCategory->image['currentImage']] ) }}"
                                        alt="" width="100" height="50">
                                </td>
                                {{--                            <td>{{ $systemCategory->tags }}</td>--}}
                                <td>
                                    <label>
                                        <input id="{{ $systemCategory->id }}"
                                               onchange="changeStatus({{ $systemCategory->id }})"
                                               data-url="{{ route('admin.smart-assemble.category.status', $systemCategory->id) }}"
                                               type="checkbox" @if ($systemCategory->status === 1)
                                                   checked
                                            @endif>
                                    </label>
                                </td>
                                ...
                                <td class="width-8-rem text-left">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                            <a href="{{ route('admin.smart-assemble.category.edit', $systemCategory->id) }}"
                                               class="dropdown-item text-right"><i class="fa fa-edit"></i> ویرایش</a>
                                            <form class="d-inline"
                                                  action="{{ route('admin.smart-assemble.category.destroy', $systemCategory->id) }}"
                                                  method="post">
                                                @csrf
                                                @method('Delete')
