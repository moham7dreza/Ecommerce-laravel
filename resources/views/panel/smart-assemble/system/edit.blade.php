    ...
    <section class="row">
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section>
                    <form action="{{ route('admin.smart-assemble.system.update', $system->id) }}" method="post"
                        ...
                        <section class="row">
                                    ...
                                    <select name="system_config_id" id="" class="form-control form-control-sm">
                                        @foreach ($configs as $config)
                                            <option value="{{ $config->id }}"
                                                    @if(old('system_config_id', $system->system_config_id) == $config->id) selected @endif>{{ $config->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="system_gen_id" id="" class="form-control form-control-sm">
                                        @foreach ($gens as $gen)
                                            <option value="{{ $gen->id }}"
                                                    @if(old('system_gen_id', $system->system_gen_id) == $gen->id) selected @endif>{{ $gen->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('system_gen_id')
                                    ...
                                    <select name="system_type_id" id="" class="form-control form-control-sm">
                                        @foreach ($types as $type)
                                            <option value="{{ $type->id }}"
                                                    @if(old('system_type_id', $system->system_type_id) == $type->id) selected @endif>{{ $type->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="system_category_id" id="" class="form-control form-control-sm">
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                    @if(old('system_category_id', $system->system_category_id) == $category->id) selected @endif>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="system_user_type"
                                           value="{{ old('system_user_type', $system->system_user_type) }}"
                            ...
                            <section class="row">
                                @php
                                    $number = 1;
                                @endphp
                                @foreach ($system->image['indexArray'] as $key => $value )
                                    <section class="col-md-{{ 6 / $number }}">
                                        <div class="form-check">
                                            <input type="radio" class="form-check-input" name="currentImage"
                                                   value="{{ $key }}" id="{{ $number }}"
                                                   @if($system->image['currentImage'] == $key) checked @endif>
                            ...
                            <section class="col-12 col-md-6 my-2">
                                <div class="form-group">
                                    ...
                                    <select name="status" id="" class="form-control form-control-sm" id="status">
                                        <option value="0" @if (old('status', $system->status) == 0) selected @endif>
                                            غیرفعال
                                        </option>
                                        <option value="1" @if (old('status', $system->status) == 1) selected @endif>
                                            فعال
                                        </option>
                                    </select>
                                </div>
                            ...
                            <section class="col-12 col-md-6 my-2">
                                <div class="form-group">
                                    ...
                                    <select name="system_marketable" class="form-control form-control-sm"
                                            id="marketable">
                                        <option value="0"
                                                @if(old('system_marketable', $system->system_marketable) == 0) selected @endif>
                                            غیرفعال
                                        </option>
                                        <option value="1"
                                                @if(old('system_marketable', $system->system_marketable) == 1) selected @endif>
                                            فعال
                                        </option>
                                    </select>
                                </div>
