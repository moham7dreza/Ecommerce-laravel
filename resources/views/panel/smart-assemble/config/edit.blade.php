    ...
    <section class="row">
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section>
                    <form action="{{ route('admin.smart-assemble.config.update', $systemConfig->id) }}" method="post"
                        ...
                        <section class="row">
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" value="{{ old('name', $systemConfig->name) }}"
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="system_gen_id" id="" class="form-control form-control-sm">
                                        @foreach ($gens as $gen)
                                            <option value="{{ $gen->id }}"
                                                    @if(old('system_gen_id', $systemConfig->system_gen_id) == $gen->id) selected @endif>{{ $gen->name }}</option>
                                        @endforeach
                                    ...
                                    <select name="system_type_id" id="" class="form-control form-control-sm">
                                        @foreach ($types as $type)
                                            <option value="{{ $type->id }}"
                                                    @if(old('system_type_id', $systemConfig->system_type_id) == $type->id) selected @endif>{{ $type->name }}</option>
                                        @endforeach
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="start_price_from"
                                           value="{{ old('start_price_from', $systemConfig->start_price_from) }}"
                                           class="form-control form-control-sm">
                                </div>
                            ...
                            <section class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="brief" value="{{ old('brief', $systemConfig->brief) }}"
                                           class="form-control form-control-sm">
                                </div>
                                @error('brief')
                                <span class="alert_required bg-danger text-white p-1 rounded" role="alert">
                                    ...
                                    <section class="col-md-{{ 6 / $number }}">
                                        <div class="form-check">
                                            <input type="radio" class="form-check-input" name="currentImage"
                                                   value="{{ $key }}" id="{{ $number }}"
                                                   @if($systemConfig->image['currentImage'] == $key) checked @endif>
                            ...
                            <section class="col-12 col-md-6 my-2">
                                <div class="form-group">
                                    ...
                                    <select name="status" id="" class="form-control form-control-sm" id="status">
                                        <option value="0"
                                                @if (old('status', $systemConfig->status) == 0) selected @endif>غیرفعال
                                        </option>
                                        <option value="1"
                                                @if (old('status', $systemConfig->status) == 1) selected @endif>فعال
                                        </option>
                                    </select>
                                </div>
                                @error('status')
                            ...
                            <section class="col-12 col-md-6 my-2">
                                <div class="form-group">
                                    ...
                                    <select name="show_in_menu" id="" class="form-control form-control-sm"
                                            id="show_in_menu">
                                        <option value="0"
                                                @if (old('show_in_menu', $systemConfig->show_in_menu) == 0) selected @endif>
                                            غیرفعال
                                        </option>
                                        <option value="1"
                                                @if (old('show_in_menu', $systemConfig->show_in_menu) == 1) selected @endif>
                                            فعال
                                        </option>
                                    </select>
                                </div>
