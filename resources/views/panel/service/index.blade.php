                            ...
                            <table class="table table-striped mb-0">
                                ...
                                @foreach($services as $service)
                                    <tr class="text-center">
                                        ...
                                        <td>{{ $service->getParent() }}</td>

                                        <td>{{ jalaliDate($service->created_at) }}</td>
                                        <td>
                                            <div class="row">
                                                <a href="{{ route('panel.service.edit', $service->id) }}" class="btn outlined c-main o-main fnt-xxs ml-1">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </a>
                                                <form action="{{ route('panel.service.change.status', $service->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="{{ route('panel.service.destroy', $service->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
