@extends('customer.layouts.filter-two-col')
@section('head-tag')
    <title>
       فیلتر
    </title>
...
@foreach($productsWithActiveAmazingSales as $offer)
            <section class="col-md-3 p-0">
                <section class="product">
                    ...
                    <a class="product-link" href="{{ route('customer.market.product', $offer->product) }}">
                        <section class="product-image">
                            <img class="" src="{{ asset($offer->product->image['indexArray']['medium']) }}" alt="{{ $offer->product->name }}">
                        </section>
                        <section class="product-colors"></section>
                        <section class="product-name"><h3>{{ $offer->product->name }}</h3></section>
                        <section class="product-price-wrapper">
                            <section class="product-price">{{ priceFormat($offer->product->price) }} تومان</section>
                        </section>
                        <section class="product-colors">
                            @foreach ($offer->product->colors()->get() as $color)
