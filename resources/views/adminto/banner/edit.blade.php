                        ...
                        <div class="col-12">
                            <div class="p-2">
                                <form class="form-horizontal" role="form" method="POST"
                                      action="{{ route('adminto.banner.update', $banner->id) }}"
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control @error('title') is-invalid @enderror"
                                                   value="{{ $banner->title }}" id="title" name="title"
                                                   placeholder="عنوان تبلیغات را وارد کنید">
                                            @error('title')
                                            <br>
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control @error('position') is-invalid @enderror"
                                                ...
                                                @foreach (\App\Models\Content\Banner::$postBannersPositions as $key => $position)
                                                    <option @if (old('position', $banner->position) === $key) selected
                                                            @endif
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control @error('status') is-invalid @enderror"
                                                ...
                                                @foreach (\App\Models\Content\Banner::$statuses as $status)
                                                    <option value="{{ $status }}"
                                                            @if(old('status', $banner->status) == $status) selected @endif>
                                                        @if($status == 1)
                                                            فعال
                                                        @else
                                                            غیر فعال
