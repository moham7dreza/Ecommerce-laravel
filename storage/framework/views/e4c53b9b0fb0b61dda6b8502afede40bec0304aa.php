...
>
    <div class="mx-3 mb-2">
        ...
        <?php if($backUrl = config('log-viewer.back_to_system_url')): ?>
            <a href="<?php echo e($backUrl); ?>" class="rounded inline-flex items-center text-sm text-gray-400 hover:text-emerald-800 dark:hover:text-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-700 mt-3">
    ...
    <?php $__currentLoopData = $folderCollection; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $folder): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div x-data="{ folder: '<?php echo e($folder->identifier); ?>' }" :id="'folder-'+folder"
             ...
             x-init="$nextTick(() => { if (<?php echo json_encode($folder->isRoot() && empty($selectedFileIdentifier), 15, 512) ?>) { $store.fileViewer.foldersOpen.push('<?php echo e($folder->identifier); ?>') } })"
        >
            ...
            >
                <div class="file-item">
                    ...
                    <div class="file-name"><?php if(\Illuminate\Support\Str::startsWith($folder->cleanPath(),'root')): ?><span class="text-gray-500 dark:text-gray-400">root</span><?php echo e(substr($folder->cleanPath(), 4)); ?><?php else: ?><?php echo e($folder->cleanPath()); ?><?php endif; ?></div>
                ...
                >
                    <div class="py-2">
                        <button wire:click="clearFolderCache('<?php echo e($folder->identifier); ?>')" x-on:click.stop="cacheRecentlyCleared = false;" x-data="{ cacheRecentlyCleared: <?php echo json_encode($cacheRecentlyCleared, 15, 512) ?> }"
                        ...
                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('downloadLogFolder', $folder)): ?>
                        <a href="<?php echo e($folder->downloadUrl()); ?>" x-on:click.stop="">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><use href="#icon-download" /></svg>
                            Download
                        </a>
                        <?php endif; ?>

                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('deleteLogFolder', $folder)): ?>
                        <div class="divider"></div>
                        <button x-on:click.stop="if (confirm('Are you sure you want to delete the log folder \'<?php echo e($folder->path); ?>\'? THIS ACTION CANNOT BE UNDONE.')) { $wire.call('deleteFolder', '<?php echo e($folder->identifier); ?>') }" wire:loading.attr="disabled">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" wire:loading.class.remove="hidden" wire:target="deleteFolder" wire:target="deleteFolder('<?php echo e($folder->identifier); ?>')" class="hidden spin" fill="currentColor"><use href="#icon-spinner" /></svg>
                            Delete
                        </button>
                        <?php endif; ?>
                    </div>
...
<?php /**PATH C:\CODEX\techzilla\vendor\opcodesio\log-viewer\src/../resources/views/livewire/file-list.blade.php ENDPATH**/ ?>