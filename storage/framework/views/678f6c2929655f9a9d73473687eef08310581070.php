...
>
    <div class="file-item grow">
        ...
        <div class="my-auto mr-2" x-show="$store.fileViewer.checkBoxesVisibility">
            <input type="checkbox"
                x-init="$watch(`$store.fileViewer.isChecked('<?php echo e($logFile->identifier); ?>')`, (value) => {
                    $el.checked = value;
                })"
                x-on:click.stop="$store.fileViewer.checkBoxToggle('<?php echo e($logFile->identifier); ?>')"
                value="<?php echo e($logFile->identifier); ?>" />
        </div>
        <?php endif; ?>
    ...
    >
        <div class="py-2">
            ...
            <button x-on:click.stop="if (confirm('Are you sure you want to delete the log file \'<?php echo e($logFile->name); ?>\'? THIS ACTION CANNOT BE UNDONE.')) { $wire.call('deleteFile', '<?php echo e($logFile->identifier); ?>') }">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><use href="#icon-trashcan" /></svg>
                Delete
            </button>
            <button x-on:click.stop="$store.fileViewer.showCheckBoxes(); $store.fileViewer.checkBoxToggle('<?php echo e($logFile->identifier); ?>'); close($refs.button)">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><use href="#icon-trashcan" /></svg>
                Delete Multiple
            </button>
            <?php endif; ?>
...
<?php /**PATH C:\CODEX\techzilla\vendor\opcodesio\log-viewer\src/../resources/views/partials/file-list-item.blade.php ENDPATH**/ ?>