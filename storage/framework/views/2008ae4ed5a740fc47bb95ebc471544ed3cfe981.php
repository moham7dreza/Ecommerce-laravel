    ...
    <div class="flex flex-col h-full w-full mx-3 mb-4">
        <div class="px-4 mb-4 flex items-start">
            <div class="flex items-center mr-6">
                <?php if(isset($showLevelsDropdown) && $showLevelsDropdown): ?>
        ...
        <?php if(isset($logs) && ($logs->isNotEmpty() || !$hasMoreResults)): ?>
...
<?php else: ?> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><use href="#icon-info" /></svg><?php endif; ?>
        ...
        <td class="whitespace-nowrap text-gray-900 dark:text-gray-200"><?php echo \Opcodes\LogViewer\Utils\Utils::highlightSearchResult($log->time->toDateTimeString(), $query); ?></td>
        <td class="whitespace-nowrap text-gray-500 dark:text-gray-300 dark:opacity-90 hidden lg:table-cell"><?php echo \Opcodes\LogViewer\Utils\Utils::highlightSearchResult($log->environment, $query); ?></td>
        <td class="max-w-[1px] w-full truncate text-gray-500 dark:text-gray-300 dark:opacity-90"><?php echo \Opcodes\LogViewer\Utils\Utils::highlightSearchResult($log->text, $query); ?></td>
    ...
    <tr x-show="$store.logViewer.isOpen(<?php echo e($index); ?>)"><td colspan="6">
        <pre class="log-stack"><?php echo \Opcodes\LogViewer\Utils\Utils::highlightSearchResult($log->fullText, $query); ?></pre>
        <?php if($log->fullTextIncomplete): ?>
        <div class="py-4 px-8 text-gray-500 italic">
            The contents of this log have been cut short to the first <?php echo e(\Opcodes\LogViewer\Utils\Utils::bytesForHumans(\Opcodes\LogViewer\Facades\LogViewer::maxLogSize())); ?>.
    ...
    <td colspan="6">
        <div class="bg-white text-gray-600 dark:bg-gray-800 dark:text-gray-200 p-12">
            ...
            <div class="text-center mt-6">
                ...
                <?php endif; ?>
                <?php if(isset($levels) && count(array_filter($levels, fn ($level) => $level->selected)) === 0 && count(array_filter($levels, fn ($level) => $level->count > 0))): ?>
...
<?php /**PATH C:\CODEX\techzilla\vendor\opcodesio\log-viewer\src/../resources/views/livewire/log-list.blade.php ENDPATH**/ ?>